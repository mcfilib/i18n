...
decode :: String -> String -> Either String (Set String)
decode keyword src =
  case parseDoc defaultHamletSettings src of
    Ok (_, docs)  -> Right $ Set.fromList (findTranslations keyword docs)
  ...
  where
    ...
    findTranslations x xs =
      [body | DocContent (ContentVar (DerefBranch (DerefIdent (Ident i)) (DerefString body))) <- xs, i == x]
